...
const checkpoints = [
  ...
  { id: "pit1", label: "PIT STOP 1", sub: "ACTIVITIES" },
  { id: "pit2", label: "PIT STOP 2", sub: "PACKAGES" },
  { id: "pit3", label: "PIT STOP 3", sub: "GALLERY" },
  { id: "pit4", label: "PIT STOP 4", sub: "EVENTS" },
  { id: "pit5", label: "PIT STOP 5", sub: "REVIEWS" },
  { id: "finish", label: "FINISH", sub: "CONTACT" },
];
...
export default function RacingTrack() {
  ...
  useEffect(() => {
    ...
    const unsubscribe = scrollYProgress.on("change", (v) => {
      ...
      lastValue = v;
      const idx = Math.min(Math.floor(v * checkpoints.length), checkpoints.length - 1);
      setCurrentCheckpoint(idx);
    });
    return () => unsubscribe();
  }, [scrollYProgress]);
  ...
  return (
    <div ref={trackRef} className="fixed left-0 top-0 bottom-0 w-10 md:w-12 z-30 pointer-events-none">
      {/* Track surface */}
      <div className="absolute inset-y-0 left-1/2 -translate-x-1/2 w-[3px] bg-[#080808] opacity-40">
        {/* Lane lines */}
        <div className="absolute left-0 top-0 bottom-0 w-px bg-[repeating-linear-gradient(to_bottom,rgba(255,30,30,0.1)_0px,rgba(255,30,30,0.1)_8px,transparent_8px,transparent_16px)]" />
        <div className="absolute right-0 top-0 bottom-0 w-px bg-[repeating-linear-gradient(to_bottom,rgba(255,30,30,0.1)_0px,rgba(255,30,30,0.1)_8px,transparent_8px,transparent_16px)]" />
        {/* Center line */}
        <div className="absolute left-1/2 -translate-x-1/2 top-0 bottom-0 w-px bg-[repeating-linear-gradient(to_bottom,rgba(255,255,255,0.06)_0px,rgba(255,255,255,0.06)_4px,transparent_4px,transparent_8px)]" />
      </div>

      {/* Checkpoints */}
      {checkpoints.map((cp, i) => {
        const isActive = currentCheckpoint === i;
        return (
          <div
            key={cp.id}
            className="absolute left-1/2 -translate-x-1/2 flex items-center"
            style={{ top: `${(i / (checkpoints.length - 1)) * 100}%` }}
          >
            <motion.div
              className={`w-[3px] h-[3px] rounded-full transition-all duration-300 ${
                isActive ? "bg-accent scale-[2.5]" : "bg-white/8"
              }`}
              animate={isActive ? { scale: [1, 2, 1] } : {}}
              transition={{ duration: 1.5, repeat: Infinity }}
            />
            <div
              className={`absolute left-2.5 top-1/2 -translate-y-1/2 whitespace-nowrap transition-all duration-300 ${
                isActive ? "opacity-25" : "opacity-8"
              }`}
            >
              <div
                className={`text-[6px] md:text-[7px] font-bold tracking-widest ${
      ...
      >
        <motion.div
          style={{
            rotate: scrollVelocity > 2 ? (scrollVelocity > 4 ? 1.5 : 0.8) : 0,
          }}
